...
from aggregators import SumAggregator, LabelAggregator,SumAggregator_2
...
class my_KGNN_LS(object):
    def __init__(self, args, n_user, n_entity, n_relation, adj_entity, adj_relation, interaction_table, offset):
        self._parse_args(args, adj_entity, adj_relation, interaction_table, offset)
        self._build_inputs()
        self._build_model(n_user, n_entity, n_relation)
        self._build_train()
        print('use my_KGNN_LS')
    ...
    def aggregate(self, entities, relations):
        ...
        for i in range(self.n_iter):
            if i == self.n_iter - 1:
                aggregator = SumAggregator_2(self.batch_size, self.dim, act=tf.nn.tanh)
            else:
                aggregator = SumAggregator_2(self.batch_size, self.dim)
            aggregators.append(aggregator)
            ...
            for hop in range(self.n_iter - i):
                ...
                #                     masks=None)
                # entity_vectors_next_iter.append(vector)
                # NOTICE 改法1，做两次agg相加——失败，可能是vector2维度比vector1大
                shape1 = [self.batch_size, -1, self.n_neighbor, self.dim]
                shape2 = [self.batch_size, -1, self.n_neighbor*self.n_neighbor, self.dim]
                vector1 = aggregator(self_vectors=entity_vectors[hop],
                                    neighbor_vectors=tf.reshape(entity_vectors[hop + 1], shape1),
                                    neighbor_relations=tf.reshape(relation_vectors[hop], shape1),
                                    user_embeddings=self.user_embeddings,
                                    masks=None)
                vector2 = aggregator(self_vectors=entity_vectors[hop],
                                    neighbor_vectors=tf.reshape(entity_vectors[hop + 2], shape2),
                                    neighbor_relations=tf.reshape(relation_vectors[hop+1], shape2),
                                    user_embeddings=self.user_embeddings,
                                    masks=None)
                # entity_vectors_next_iter.append(vector2)#失败，估计是vector2维度刚好是vector1的8倍
                entity_vectors_next_iter.append(vector1+vector2)
